...
public class ActivityMuestraDespacho extends AppCompatActivity {
   ...
   // private Animator spruceAnimator;
    ...
    private static String TAG = ActivityMuestraDespacho.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_muestra_despacho);
        ...
        DespachoVO despachoVO = new DespachoDAO(getBaseContext()).buscarById((long) MUESTRA.getIdProceso());

        tViewFechaInspeccion.setText(despachoVO.getFechaInspeccion());
        tViewProducto.setText(despachoVO.getNameCultivo());
        ...
        evaluacionDetalleVOList = new EvaluacionDetalleDAO(getBaseContext()).listByIdMuestra(MUESTRA.getId());
            ...
            handler.post(new Runnable() {
                public void run() {
                    ...
                    adapterListEvaluacionDetalle = new RViewAdapterEvaluacionDetalle_new(ActivityMuestraDespacho.this,evaluacionDetalleVOList, rViewEvaluacionesDetalle,isEditable,eTextNFrutos);
...
//hacer q  el edit  text pieda el foco cando se preciosne  en toro lado
    ...
    public void onBackPressed() {
        ...
        int cantEva = new EvaluacionDetalleDAO(getBaseContext()).listByIdMuestra(MUESTRA.getId()).size();
        Log.d(TAG,""+cantEva);
        if(/*saveMuestras.size()==0 ||*/ cantEva==0){
...
*/
    ...
    public void showListEvaluaciones(View view){
        if(MUESTRA.getIdTipoProceso()>0){
            if(isEditable){
                List<EvaluacionVO> evaluaciones = new EvaluacionDAO(getBaseContext()).listByIdTipoProcesoIdMuestraDespacho(MUESTRA.getIdTipoProceso(),MUESTRA.getId());
