...
import static com.ibao.agroq.utilities.Utilities.URL_DOWNLOAD_TABLE_EVALUACION;

public class DownloaderEvaluacion {

    String TAG = "DOWN Evaluacion";
    Context ctx;
 //   ProgressDialog progress;
    public static int status;
    public static int idCultivo = 0;
    public DownloaderEvaluacion(Context ctx){
        status=0;
        this.ctx = ctx;
    }
     ...
     //                   progress.dismiss();
                        try {
                            ...
                            String insert = "INSERT INTO " +
                                    TABLE_EVALUACION+
                                    "("+
                                    TABLE_EVALUACION_COL_ID+","+
                                    TABLE_EVALUACION_COL_NAME+","+
                                    TABLE_EVALUACION_COL_ISMATSEC+","+
                                    TABLE_EVALUACION_COL_CULTIVO+
                                    ")"+
                                    "VALUES ";
                            for(int i=0;i<main.length();i++){
                                ...
                                int isMateriaSeca = data.getInt("isMateriaSeca");
                                int idCultivo = data.getInt("idCultivo");
                                Log.d(TAG,"fila "+i+" : "+id+" "+nombre);
                                /*
                                if(new ZonaDAO(ctx).insertarZona(id,nombre)){

                                    Log.d(TAG,"logro insertar"+id);
                                }
                                */
                                insert=insert+"("+id+",\""+nombre+"\","+isMateriaSeca+","+idCultivo+")";
                                if(i%1000==0&& i>0){
                                    try{
       ...
       //                 progress.dismiss();
            ...
            protected Map<String,String> getParams(){
                ...
                //params.put("idCultivo",String.valueOf(new LoginDataDAO(ctx).verficarLogueo().getIdCultivo()));
                return params;
            }
