...
import static com.ibao.agroq.utilities.Utilities.URL_DOWNLOAD_TABLE_CATEGORIA;

public class DownloaderCategoria {

    Context ctx;
 //   ProgressDialog progress;
    public static int status;
    public DownloaderCategoria(Context ctx){
        status=0;
        this.ctx = ctx;
    }
   ...
   //     progress.show();
     ...
     //                   progress.dismiss();
                        try {
                            ...
                            if(main.length()>0){
                                new CategoriaDAO(ctx).clearTableUpload();
                                status=2;
                            }
                            String insert = "INSERT INTO " +
                                    TABLE_CATEGORIA+
                                    "("+
                                    TABLE_CATEGORIA_COL_ID+","+
                                    TABLE_CATEGORIA_COL_NAME+
                                    ")"+
                                    "VALUES ";
