...
import com.ibao.agroq.models.dao.TipoProcesoDAO;
...
import static com.ibao.agroq.utilities.Utilities.URL_DOWNLOAD_TABLE_TIPOPROCESO;

public class DownloaderTipoProceso {

    String TAG = "Down TipoProcesoVO";
    Context ctx;
 //   ProgressDialog progress;
    public static int status;
    public DownloaderTipoProceso(Context ctx){
        status=0;
        this.ctx = ctx;
    }
   ...
   //     progress.show();
        StringRequest sr = new StringRequest(Request.Method.POST,
                URL_DOWNLOAD_TABLE_TIPOPROCESO,
     ...
     //                   progress.dismiss();
                        try {
                            ...
                            if(main.length()>0){
                                new TipoProcesoDAO(ctx).clearTableUpload();
                                status=2;
                            }
                            ...
                            String insert = "INSERT INTO " +
                                    TABLE_TIPOPROCESO+
                                    "("+
                                    TABLE_TIPOPROCESO_COL_ID+","+
                                    TABLE_TIPOPROCESO_COL_NAME+
                                    ")"+
                                    "VALUES ";
        ...
        */
        ...
        StringRequest sr = new StringRequest(Request.Method.POST,
                ...
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        mensaje.setText("Descargando Tipo Procesos");
                       // progress.dismiss();
                        try {
                            JSONArray main = new JSONArray(response);
                            ...
                            for(int i=0;i<main.length();i++){
                                ...
                                if(new TipoProcesoDAO(ctx).insertar(id,nombre)){
