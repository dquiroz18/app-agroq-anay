...
import static com.ibao.agroq.utilities.Utilities.URL_DOWNLOAD_TABLE_DESTINO;

public class DownloaderDestino {

    Context ctx;
 //   ProgressDialog progress;
    public static int status;
    public DownloaderDestino(Context ctx){
        status=0;
        this.ctx = ctx;
    }
   ...
   //     progress.show();
     ...
     //                   progress.dismiss();
                        try {
                            ...
                            if(main.length()>0){
                                new DestinoDAO(ctx).clearTableUpload();
                                status=2;
                            }
                            String insert = "INSERT INTO " +
                                    TABLE_DESTINO+
                                    "("+
                                    TABLE_DESTINO_COL_ID+","+
                                    TABLE_DESTINO_COL_NAME+
                                    ")"+
                                    "VALUES ";
                            for(int i=0;i<main.length();i++){
                                ...
                                Log.d("DESTINO down","fila "+i+" : "+id+" "+nombre);
                                /*
